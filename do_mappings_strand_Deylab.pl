...
if ($i){
    $str = "/home/piscopio/bwa/bwa-0.7.15/bwa index -a ".$ind." ".$r;
    print $str."\n";
...
if ( $npr != 2 ){
	...
	$str = "/home/piscopio/bwa/bwa-0.7.15/bwa aln -B ".$B." -q ".$q." -n ".$aln_n." -k ".$aln_k." -l ".$l." -t ".$t." ".$r." ".$G[$i]." > ".$H[$i];
	print $str."\n";
	(system($str) == 0 or die "Could not execute ".$str."\n") if ($test == 0);
    }
    ...
    if ( $nsam == 0 ){
	if ($pflag){
	    $str = "/home/piscopio/bwa/bwa-0.7.15/bwa sampe -n ".$n." -N ".$N." ".$r." ".join(" ",(@H, @G))." > ".$out.".sam";
	}else{
	    $str = "/home/piscopio/bwa/bwa-0.7.15/bwa samse -n ".$n." ".$r." ".join(" ",(@H, @G))." > ".$out.".sam";
	}
    }
    print $str."\n";
    (system($str) == 0 or die "Could not execute ".$str."\n") if ($test == 0);
